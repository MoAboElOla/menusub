...
export default function Success() {
    ...
    const handleNew = () => { clearAuth(); localStorage.removeItem('zipDownloadUrl'); localStorage.removeItem('excelDownloadUrl'); navigate('/'); };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4">
            <div className="w-full max-w-md text-center">
                ...
                <p className="text-gray-400 mb-8">{t('successSubtitle')}</p>
                ...
                <div className="space-y-3 mb-8">
                    ...
                    {excelUrl && (
                        <a href={excelUrl} className="flex items-center justify-center gap-3 w-full py-4 bg-[#181924] text-gray-200 border border-gray-700/50 hover:border-emerald-500/50 hover:text-emerald-400 font-semibold rounded-xl transition-all active:scale-[0.98]">
                ...
                <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 mb-8">
                    <div className="flex items-start gap-2">
                        ...
                        <p className="text-sm text-amber-300/80 text-start">
                            {t('retentionWarning')} <strong className="text-amber-300">72 {t('hours')}</strong>. {t('downloadNow')}
                        </p>
                    </div>
                </div>

                <button onClick={handleNew} className="text-gray-500 hover:text-brand-400 text-sm transition-colors">{t('newSubmission')}</button>
            </div>
        </div>
    );
}
